...
namespace learnfft
{
    ...
    void FFTRadix2SIMD<T>::Forward(const T* real_in, const T* imag_in, T* real_out, T* imag_out)
    {
        ...
        FFTRadix2SIMDCore3(real_in, imag_in, real_out, imag_out, true);
    }
    ...
    void FFTRadix2SIMD<T>::Inverse(const T* real_in, const T* imag_in, T* real_out, T* imag_out)
    {
        ...
        FFTRadix2SIMDCore3(real_in, imag_in, real_out, imag_out, false);
    }
            ...
            for (int i = 0; i < m_size; i += btfly)
            {
                if (step < 4) {
                    ...
                    for (int k = 0; k < step; k+=4)
                    {
                        ...
                        __m256d real_out_odd_256d = _mm256_loadu_pd(real_out + odd);
            ...
            for (int i = 0; i < m_size; i += btfly)
            {
                if (step < 2) {
                    ...
                    for (int k = 0; k < step; k+=2)
                    {
                        ...
                        _mm_storeu_pd(real_out + odd, real_out_odd_128d);
